...
You can view the messages the device sends with the Arduino "Serial Monitor", by knowing the COMx or ttyUSBx device and it's baud rate (it is `115200 baud` by default). After programming code that uses the default serial terminal, you can go to **Tools** > **Serial Monitor** or press CTRL+SHIFT+\[M\].
...
If you want to know more about Serial connections, you might read about [UART](https://en.wikipedia.org/wiki/Universal_asynchronous_receiver-transmitter), [why serial ports are named the way they are](https://superuser.com/questions/1207025/why-are-usb-ports-sometimes-referred-to-as-serial-ports-and-called-com), and and you can even use free tools and read documents to learn more than you ever wanted to know about USB and how it works at [usb.org](https://usb.org/documents).
